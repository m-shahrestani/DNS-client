...
url = input()
msgFromClient = build_message("A", url)
...
try:
    UDPClientSocket.sendto(binascii.unhexlify(msgFromClient), serverAddressPort)
    data, _ = UDPClientSocket.recvfrom(4096)
finally:
    UDPClientSocket.close()

# Send to server using created UDP socket
msgFromServer = binascii.hexlify(data).decode("utf-8")

print("Message from DNS Server {}".format(msgFromServer))
print((decode_message(msgFromServer)))